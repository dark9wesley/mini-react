...
> 当 react 在 render 阶段时，会给变化的 fiber 节点打上不同 tag。后续 commit 阶段，会根据这些 tag 来操作 DOM。

当一个`fiber`节点没有对应的`current节点`时，可以认定为它是一个新挂载的节点，这时候需要给它打上`Placement（新增）`的`tag`。

当一个节点是新挂载的节点时，按道理来说，它的所有子节点也都是新增的节点。如果给它们都打上`Placement（新增）`的`tag`，那么要操作的新增 DOM 就太多了。

如果我们在`render阶段`就构建一颗离屏 DOM 树，最后只需要将根节点插入真实 DOM，这样性能就会大大提升。

所以这里做了逻辑分发，区分了两个逻辑：

1. 当 current 树存在时，后续子节点处于**更新**阶段，子节点需要被打上`tag`。
2. 当 current 树不存在时，后续子节点处于**挂载**阶段，子节点不需要被打上 `tag`。
...
`shouldTrackEffect`就是是否追踪副作用的意思，这决定了是否需要给`fiber`打上 `tag`。
...
当节点不可被复用时，会调用该方法，用来给`fiber`节点打上`ChildDeletion tag`
...
该方法主要用来给 `fiber` 节点打上 `Placement tag`
...
可以看到当`shouldTrackEffect === true`和`fiber.alternate`不存在时，才会给`fiber` 节点打上 `Placement tag`。
...
3. 它的子节点是一个新挂载的节点，所以没有`alternate`，这时`shouldTrackEffect === true`和`fiber.alternate === null`都成立，就会给这个节点打上`Placement tag`。
4. 同时由于新挂载的子节点没有`alternate`，所以后续在协调它的子节点时，都会调用`mountChildFibers`,也就是`shouldTrackEffect`为`false`，后续的子节点都不会打上`Placement tag`。

总的来说，`Placement tag`只会给挂载的头节点，节省操作资源。
